...
# code: https://github.com/qitianwu/NodeFormer/blob/main/nodeformer.py
BIG_CONSTANT = 1e8
...
def kernelized_gumbel_softmax(
        query, key, value, kernel_transformation, projection_matrix=None, edge_index=None,
...
class NodeFormerConv(nn.Module):
    ...
    def __init__(
            ...
            kernel_transformation=softmax_kernel_transformation, projection_matrix_type='a',
    ...
    def forward(self, z, adjs, tau):
        ...
        if self.use_gumbel and self.training:  # only using Gumbel noise for training
            z_next, weight = kernelized_gumbel_softmax(
                query=query, key=key, value=value, kernel_transformation=self.kernel_transformation,
                projection_matrix=projection_matrix, edge_index=adjs[0],
                K=self.nb_gumbel_sample, tau=tau, return_weight=self.use_edge_loss
            )
        else:
            z_next, weight = kernelized_softmax(
                query=query, key=key, value=value, kernel_transformation=self.kernel_transformation,
                projection_matrix=projection_matrix, edge_index=adjs[0],
                tau=tau, return_weight=self.use_edge_loss
            )
...
class NodeFormer(nn.Module):
    ...
    '''
    def __init__(
            self, in_channels, hidden_channels, out_channels, num_layers=2, num_heads=4, dropout=0.0,
            kernel_transformation=softmax_kernel_transformation, nb_random_features=30, use_bn=True,
            use_gumbel=True, use_residual=True, use_act=False, use_jk=False, nb_gumbel_sample=10,
